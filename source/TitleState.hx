...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		bg = new FlxSprite(0, -87);
		bg.loadGraphic(Paths.image("menuassets/start/back"));
	...
	var moveModeEnabled:Bool = false;

	override function update(elapsed:Float)
	{
		if ((controls.UI_LEFT_P || controls.UI_DOWN_P || controls.UI_UP_P || controls.UI_RIGHT_P) && !moveModeEnabled && skippedIntro) {
			moveModeEnabled = true;
			FlxTween.tween(person, { 'scale.x': 4, 'scale.y': 4, alpha: 0 }, 1, { 
				ease: FlxEase.circOut,
				type: ONESHOT
			});
			FlxTween.tween(logoBl, { x: -1000 }, 1, { 
				ease: FlxEase.circOut,
				type: ONESHOT
			});
			FlxTween.tween(logoBl2, { x: FlxG.width + 1000 }, 1, { 
				ease: FlxEase.circOut,
				type: ONESHOT
			});
	...
	override function beatHit()
	{
		...
		if(person != null && bg != null) {
			if(!done && !moveModeEnabled) {
				person.scale.y = 0.89;
				person.y = 0;
				bg.scale.set(1.1, 1.1);
				FlxTween.tween(bg, { 'scale.y': 1, 'scale.x': 1 }, Conductor.crochet / 1000, {
